...
const AwayAnaly = (props) => {
    const { parsed, awayTeam } = props;
                    ...
                    <thead>
                        <tr>
                            <th width="100%" height="20" colSpan="8" className="hand-bg" align="center" style={styles.title}>
                                <font color="white"><b>Phong độ ({awayTeam})</b></font>
                            </th>
                        </tr>
                    ...
                    <tbody>
                        {Array.isArray(jsonData) && jsonData.length > 0 ? (
                            jsonData.slice(0, 3).map((jsonStr, index) => {
                                ...
                                return (
                                    <tr key={`oddsTr_${index + 1}`} className="tb-bgcolor1" cid="8" style={{ textAlign: 'center' }}>
                                        ...
                                        <td width="10%" height="30" style={{ color: away(dataArray[4]), padding: '5px', borderBottom: '1px solid #ccc' }}>{dataArray[4]}</td>
                                        ...
                                        <td width="10%" height="30" style={{ color: away(dataArray[6]), padding: '5px', borderBottom: '1px solid #ccc' }}>{dataArray[6]}</td>
                                        <td width="2%" height="30" style={styles.cellStyle} className="HW">
                                            <span className={`o-${europe(dataArray[8], dataArray[9], dataArray[4], dataArray[6], awayTeam)}`}>
                                                {europe(dataArray[8], dataArray[9], dataArray[4], dataArray[6], awayTeam)}
                                            </span>
                                        </td>
                                        <td width="2%" height="30" style={styles.cellStyle} className="HW">
                                            <span className={`o-${handicap(dataArray[8], dataArray[9], dataArray[4], dataArray[6], awayTeam, dataArray[20])}`}>
                                                {handicap(dataArray[8], dataArray[9], dataArray[4], dataArray[6], awayTeam, dataArray[20])}
                                            </span>
                                        </td>
                                        <td width="2%" height="30" style={styles.cellStyle} className="HW">
