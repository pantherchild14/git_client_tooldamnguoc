...
const H2H = (props) => {
            ...
            const applyFilter = () => {
                if (activeFilter === "Home") {
                    const filteredByHome = data.filter((e) => {
                        const value = e.split(',');
                        return value[4] === nameTeam;
                    });
                    setFilteredData(filteredByHome);
                } else if (activeFilter === "Same League") {
                    const filteredByHome = data.filter((e) => {
                        const value = e.split(',');
                        return value[1] === league;
                    });
                    setFilteredData(filteredByHome);
                } else if (activeFilter === "Same League" && activeFilter === "Home") {
                    const filteredByLeagueAndHome = data.filter((e) => {
                        const value = e.split(',');
                        return value[1] === league && value[4] === nameTeam;
                    });
                    setFilteredData(filteredByLeagueAndHome);
                } else {
    ...
    useEffect(() => {
        ...
        first10FilteredData.map((e, index) => {
            ...
            const tdValueEU = europe(values[8], values[9], values[4], values[6], nameTeam);

            const tdValueAH = handicap(values[8], values[9], values[4], values[6], nameTeam, values[20]);
                        ...
                        <tr className="team-home" style={styles.awayHome}>
                            <td colSpan="16">
                                <div style={styles.title}>
                                    <label style={{ marginRight: '10px', color: '#fff', fontSize: '14px', fontWeight: 'bold' }}>{nameTeam}</label>
                                    ...
                                    <select value={matchCount} onChange={handleSelectMatch} style={{ cursor: 'pointer' }}>
                                ...
                                filteredData.slice(0, selectMatchCount).map((e, index) => {
                                    ...
                                    return (
                                        <tr key={index} name={`oddsTr_${index + 1}`} className="tb-bgcolor1" cid="8" style={{ textAlign: 'center' }}>
                                            ...
                                            <td width="10%" height="30" style={{ color: checkName(values[4]) }}>{values[4]}</td>
                                            ...
                                            <td width="10%" height="30" style={{ color: checkName(values[6]) }}>{values[6]}</td>
                                            {/* Europe  */}
                                            <td width="2%" height="30">{values[25]}</td>
                                            <td width="2%" height="30">{values[26]}</td>
                                            <td width="2%" height="30">{values[27]}</td>
                                            <td width="2%" height="30" >
                                                <span className={`o-${europe(values[8], values[9], values[4], values[6], nameTeam)}`}>
                                                    {europe(values[8], values[9], values[4], values[6], nameTeam)}
                                                </span>
                                            </td>
                                            {/* Handicap  */}
                                            ...
                                            <td width="2%" height="30" >
                                                <span className={`o-${handicap(values[8], values[9], values[4], values[6], nameTeam, values[20])}`}>
                                                    {handicap(values[8], values[9], values[4], values[6], nameTeam, values[20])}
                                                </span>
                                            </td>
                                            {/* Over  */}
